	...
	<div class="container color2 shadow-lg border my-3 h-auto">
		...
		<div class="d-flex justify-content-between col-12">
			<div class="d-flex col-10 justify-content-between">
				<form action="{{route('listarProducto')}}" class="d-flex col-12 me-3" method="GET">
					...
					<select id="" class="form-select m-3" name="categoriaBuscada">
						...
						@foreach ($categorias as $categoria)
							<option @if($categoriaBuscada == $categoria->CategoriaID) selected @endif
				...
				<tbody>
					...
					@forelse ($productos as $producto)
						<tr>
							...
							<td class="text-center col-auto" id="descripcionProducto">
								<p class="text-truncate">{{ $producto->Descripcion ?? '--' }}</p>
							</td>
							<td>
								<div class="d-flex justify-content-center mx-2">
									<a onclick="abrirModalEditarProducto({{$producto->ProductoID}})"
										class="btn btn-primary rounded-circle me-3">
										<i class="bi bi-pencil-square"></i>
									</a>

									<form action="{{ route('bajaProducto', ['ProductoID' => $producto->ProductoID]) }}"
										method="POST">
										@csrf
										@method('DELETE')
					...
					<div class="col-md-7">
						...
						<div class="input-group mt-3">
							...
							<div class="col-md-2 ms-4">
								...
								<input type="number" class="form-control  @error('cantidad') is-invalid @enderror"
									id="cantidad" required name="cantidad" min="0">
								{{-- @error('cantidad')
								<div class="alert alert-danger">{{ $message }}</div>
								@enderror --}}
							</div>
						...
						<div class="input-group mt-3">
							<div class="col-md-3">
								...
								<select required class="form-select @error('categoriaID') is-invalid @enderror"
									id="selectCategoria" name="categoriaID" onchange="isImport()">
							...
							<div class="col-md-3 ms-4 ">
								...
								<select class="form-select @error('importado') is-invalid @enderror"
									aria-label="Disabled select example" id="selectimportado" name="importado" disabled>
